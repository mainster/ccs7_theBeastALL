...
USBIntRegister(uint32_t ui32Base, void (*pfnHandler)(void))
{
    ...
    //
    Interrupt_register(ui32Int, pfnHandler);

    //
    // Enable the USB interrupt.
    //
    Interrupt_enable(ui32Int);
}
...
//!
//! \sa Interrupt_register() for important information about registering or
...
USBIntUnregister(uint32_t ui32Base)
{
    ...
    //
    Interrupt_disable(ui32Int);

    //
    // Unregister the interrupt handler.
    //
    Interrupt_unregister(ui32Int);
}
...
USBDevAddrSet(uint32_t ui32Base, uint32_t ui32Address)
{
    ...
    //
    HWREGB(ui32Base + USB_O_FADDR) = (uint16_t)ui32Address;
}
...
USBEndpointDataGet(uint32_t ui32Base, uint32_t ui32Endpoint,
                   uint16_t *pui8Data, uint32_t *pui32Size)
{
...
USBEndpointDataPut(uint32_t ui32Base, uint32_t ui32Endpoint,
                   uint16_t *pui8Data, uint32_t ui32Size)
{
    uint32_t ui32FIFO;
    uint16_t ui8TxPktRdy;
